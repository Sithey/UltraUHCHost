package fr.sithey.ultrauhchost.lib.sb;
...
public class FastBoard {
    ...
    public void updateLine(List<String> text) {
        if (text.size() != lines.size())
            clearLine();
        for (int i = 0; i <= text.size() - 1; i++) {
            if (text.get(i).length() <= 30)
                updateLine(i, text.get(i));
            else {
                String next = text.get(i).substring(0, 29);
                updateLine(i, next);
            }
        }
    }

    public void updateLine(Map<Integer, String> text) {
       // if (text.size() != lines.size())
            clearLine();
        for (Integer s : text.keySet()){
            if (text.get(s).length() <= 30)
                updateLine(s, text.get(s));
            else {
                String next = text.get(s).substring(0, 29);
                updateLine(s, next);
            }
        }
    }
    ...
    private void sendTeamPacketWithString(int score, String text, TeamMode mode) throws ReflectiveOperationException {
        if (mode == TeamMode.ADD_PLAYERS || mode == TeamMode.REMOVE_PLAYERS) {
            throw new UnsupportedOperationException();
        }
        ...
        if (mode == TeamMode.CREATE || mode == TeamMode.UPDATE) {
            ...
            if (text == null || text.isEmpty()) {
                prefix = getColorCode(score) + ChatColor.RESET;
            } else if (text.length() <= 16 || VersionType.V1_13.isHigherOrEqual()) {
                prefix = text;
            } else {
                // Prevent splitting color codes
                int index = text.charAt(15) == ChatColor.COLOR_CHAR ? 15 : 16;
